...
def k_fold_split_method(data_root, full_dataset, k=5):
    ...
    for fold_id, (train_idx, test_idx) in enumerate(skf.split(patient_list, patient_labels)):
        ...
        # Filter patient data dictionary
        train_data = {pid: data for pid, data in full_dataset.patient_data.items() 
                      if pid in train_patients}
        val_data = {pid: data for pid, data in full_dataset.patient_data.items() 
        ...
        # Print fold info
        train_hc = sum(1 for d in train_data.values() if d['hc_vs_pd'] == 0)
        train_pd = sum(1 for d in train_data.values() if d['hc_vs_pd'] == 1 and d['pd_vs_dd'] == 0)
        train_dd = sum(1 for d in train_data.values() if d['pd_vs_dd'] == 1)
        test_hc = sum(1 for d in val_data.values() if d['hc_vs_pd'] == 0)
        test_pd = sum(1 for d in val_data.values() if d['hc_vs_pd'] == 1 and d['pd_vs_dd'] == 0)
        test_dd = sum(1 for d in val_data.values() if d['pd_vs_dd'] == 1)
        
        print(f"\nFold {fold_id+1}/{k}:")
        print(f"  Train: {len(train_data)} patients (HC={train_hc}, PD={train_pd}, DD={train_dd})")
        print(f"  Test:  {len(val_data)} patients (HC={test_hc}, PD={test_pd}, DD={test_dd})")
...
class ParkinsonsDataset(Dataset):
                ...
                for task in self.tasks:
                    ...
                    try:
                        ...
                        if left_windows is not None and right_windows is not None:
                            ...
                            patient_tasks[task] = {
                                'left_wrist_windows': left_windows[:min_windows],
                                'right_wrist_windows': right_windows[:min_windows]
                            }
                        
                    except Exception as e:
                        print(f"Error loading task {task} for patient {patient_id}: {e}")
                        continue
                
                # Store patient data if any tasks were loaded
    ...
    def _create_index(self):
        """Create index based on sampling level"""
        self.index = []
        
        if self.sampling_level == 'window':
            # Window-level: each window is a separate sample
            for patient_id, patient_info in self.patient_data.items():
                for task_name, task_data in patient_info['tasks'].items():
                    n_windows = len(task_data['left_wrist_windows'])
        ...
        elif self.sampling_level == 'task':
            # Task-level: each task (all windows) is a sample
            for patient_id, patient_info in self.patient_data.items():
                for task_name in patient_info['tasks'].keys():
                    self.index.append({
                        'patient_id': patient_id,
                        'task_name': task_name
                    })
        
        elif self.sampling_level == 'patient':
            # Patient-level: each patient (all tasks, all windows) is a sample
            for patient_id in self.patient_data.keys():
                self.index.append({'patient_id': patient_id})
    ...
    def __getitem__(self, idx):
        ...
        hc_vs_pd = torch.LongTensor([patient_info['hc_vs_pd']])
        pd_vs_dd = torch.LongTensor([patient_info['pd_vs_dd']])
        
        if self.sampling_level == 'window':
            task_name = index_entry['task_name']
            window_idx = index_entry['window_idx']
            task_data = patient_info['tasks'][task_name]
            
            left_window = torch.FloatTensor(task_data['left_wrist_windows'][window_idx])
            right_window = torch.FloatTensor(task_data['right_wrist_windows'][window_idx])
            
            return (left_window, right_window, hc_vs_pd.squeeze(), pd_vs_dd.squeeze())
        
        elif self.sampling_level == 'task':
            task_name = index_entry['task_name']
            task_data = patient_info['tasks'][task_name]
            
            left_windows = torch.FloatTensor(task_data['left_wrist_windows'])
            right_windows = torch.FloatTensor(task_data['right_wrist_windows'])
            
            # Return as tuple
            return (left_windows, right_windows, hc_vs_pd.squeeze(), pd_vs_dd.squeeze())
        
        elif self.sampling_level == 'patient':
            return (patient_info['tasks'], None, hc_vs_pd.squeeze(), pd_vs_dd.squeeze())
    ...
    def get_train_test_split(self, k=5):
        ...
        return k_fold_split_method(self.data_root, self, k)
...
class MainModel(nn.Module):
    def __init__(
        ...
        self.positional_encoding = PositionalEncoding(model_dim, max_len=window_size)
...
################EvaluationFunctions###########
def calculate_metrics(y_true, y_pred, task_name="", verbose=True):  # Add verbose parameter
    if len(y_true) == 0:
        return {}
...
def train_model(config):
    ...
    # Load dataset with window-level sampling
    full_dataset = ParkinsonsDataset(
        data_root=config['data_root'],
        window_size=config.get('window_size', config['window_size']),  # Use window_size or timestep
        apply_dowsampling=config['apply_downsampling'],
        apply_bandpass_filter=config['apply_bandpass_filter'],
        sampling_level=config.get('sampling_level', 'window')  # ADDED: Pass sampling_level from config
    )
    
    split_type = config.get('split_type', 3)
    num_folds = config.get('num_folds', 5)  # FIXED: Get num_folds from config
    
    if split_type == 3:
        fold_datasets = full_dataset.get_train_test_split(k=num_folds)
