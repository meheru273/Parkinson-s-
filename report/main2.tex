...
figure~\ref{fig:sensor_viz} visualises the raw sensor data from a representative patient performing the \textit{HoldWeight} task, showing distinct patterns in the accelerometer and gyroscope channels that may be indicative of motor symptoms such as tremor and bradykinesia.
\begin{figure}[H]
\centering
...
to address the difficulty in parkinson's tremor and gait detection, we have established a two phase pipeline: (i)~a supervised learning phase using a dual-channel cross-attention Transformer architecture,
and (ii)~a self-supervised contrastive pre-training phase to enhance feature learning from unlabelled data.
% TODO: This section should describe the entire methodology as summary.
% Given bilateral IMU recordings $\mathbf{X} \in \mathbb{R}^{C \times T}$,
% where $C$ is the number of sensor channels and $T$ is the total length of the sequence.
% Add problem statement and motivation for the model architecture and design choices.
...
The cross-attention weights from the base model are visualised to provide interpretability into what bilateral temporal patterns the model attends to. Figure~\ref{fig:attn} shows attention weight maps from three representative samples, illustrating how each wrist channel attends to the other across time.

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[width=\textwidth]{attention_weights_sample_1.png}
    \caption{Sample 1}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[width=\textwidth]{attention_weights_sample_2.png}
    \caption{Sample 2}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[width=\textwidth]{attention_weights_sample_3.png}
    \caption{Sample 3}
\end{subfigure}
\caption{Cross-attention weight maps from the base model (Fold~1). Bright regions indicate time steps where one wrist strongly attends to the other, capturing bilateral motor asymmetry features characteristic of PD.}
\label{fig:attn}
\end{figure}
