    ...
    for fold_idx, (train_dataset, val_dataset) in enumerate(fold_datasets):
        ...
        for epoch in range(config['num_epochs']):
            ...
            if val_acc_combined > best_val_acc:
                ...
                # Save best model
                os.makedirs("checkpoints/ssm", exist_ok=True)
                torch.save({
                    ...
                    'config': config
                }, f"checkpoints/ssm/{task_name}_fold{fold_idx+1}.pth")
            else:
                patience_counter += 1
...
def run_ablation_study(config):
    ...
    with open('checkpoints/ssm/config.json', 'w') as f:
        json.dump(config_serializable, f, indent=2)
    print("✓ Config saved: checkpoints/ssm/config.json")
